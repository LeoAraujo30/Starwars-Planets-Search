...
describe('Testando o componente <App />', () => {
  it('Testando o componente <Search />', async () => {
  ...
  it('Testando o componente <FormFilters />', async () => {
    global.fetch = jest.fn().mockResolvedValue({
        json: jest.fn().mockResolvedValue(data),
      });
    render(<Provider><App /></Provider>);
    ...
    expect(inputColumn && inputComparison && inputValue && button).toBeInTheDocument();
    userEvent.type(inputValue, '30000000');
    userEvent.click(button);
  });
  it('Testando o componente <Filters />', async () => {
    global.fetch = jest.fn().mockResolvedValue({
        json: jest.fn().mockResolvedValue(data),
      });
    render(<Provider><App /></Provider>);
    ...
    userEvent.type(inputValue, '12600');
    userEvent.click(button);
    const deleteFilter = screen.getAllByRole('button', { name: 'X' });
    const deleteAllFilter = screen.getByRole('button', { name: 'Remover todas filtragens' });
    userEvent.click(deleteFilter[1]);
